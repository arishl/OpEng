...
double MonteCarlo::LSMPricer(const Option& option, const int steps, const int iterations, const int num_threads)
{
    ...
    for (int t = steps - 1; t >= 1; --t)
    {
        ...
        for (int i = 0; i < iterations; ++i) {
            if (const double intrinsic = !is_call ? option.GetStrikePrice() - paths[t][i] : paths[t][i] - option.GetStrikePrice(); intrinsic > 0.0) {
        ...
        for (size_t j = 0; j < itm_indices.size(); ++j)
        {
            ...
            if (const double exercise = !is_call ? option.GetStrikePrice() - St[j] : St[j] - option.GetStrikePrice(); exercise > continuation)
